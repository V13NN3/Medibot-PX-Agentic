...
export async function estimateHeightFromImage(
  ...
  const model = (process.env.GROK_VISION_MODEL || "grok-4.6").trim()

  try {
    const result = await askVision(imageBase64, width, height, model)
    console.log(`[camera] model=${model} → ${result.height_cm}cm (confidence ${result.confidence})`)
    if (result.height_cm === 0) {
      throw new Error("height estimation returned 0")
    }
    return {
...
If you cannot determine gender, use "unknown" and estimate weight as 70.`,
    ...
    if (!response.ok) {
      console.warn(`[camera] analyzePhoto ${model} failed: ${response.status}`)
      return { gender: "unknown", estimated_weight_kg: 70 }
    }
    ...
    console.warn(`[camera] analyzePhoto failed:`, err)
    return { gender: "unknown", estimated_weight_kg: 70 }
  }
}
